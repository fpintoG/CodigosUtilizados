...
from keras.layers import Dense, LSTM, CuDNNGRU, Dropout, Activation, TimeDistributed, Flatten
...
from textInBatch import data_generator, data_sequence



class CONV_LSTM_model():

	def __init__(self, model="", dataset_length=216783, 
				seq_length = 1, l2_lambda = 0.0001, embedding_vector_length = 32, 
	...
	def create_network(self):
		...
		# self.network.add(Dense(32, input_shape = (80, 32)))
		...
		self.network.add(Dropout(0.2))
		self.network.add(LSTM(50, kernel_initializer='glorot_uniform', 
				recurrent_initializer='orthogonal', 
				bias_initializer='zeros', 
				unit_forget_bias=True, 
				kernel_regularizer=None, 
	...
	def train(self, file):
		...
		self.network.fit(self.X_train, self.Y_train, batch_size=4,
                        ...
                        verbose=1,
                        callbacks=[EarlyStopping(monitor='val_loss', patience=100), checkpoint, tensorboard], class_weight='auto')
...
# train_scaled = train_scaled.reshape(train_scaled.shape[0], seq_length, train_scaled.shape[1])
# val_scaled = val_scaled.reshape(val_scaled.shape[0], seq_length, val_scaled.shape[1])
# test_scaled = test_scaled.reshape(test_scaled.shape[0], seq_length, test_scaled.shape[1])
	
model = CONV_LSTM_model()
model.getData(X_train, Y_train, X_val, Y_val, X_test, Y_test)
model.train("conv_lstm_model8")
model.save("conv_lstm_model8")
model.evaluate()		
